...
Run the script using the following command:
    ...
    python edge_attack_yolo_eval.py  --max_samples "max_samples" --epsilon "epsilon" --gridsize "gridsize" --iterations "iterations"
...
parser.add_argument("--epsilon", type=float, default=1, help="Epsilon value for edge_attack (default: 1)")
parser.add_argument("--grid_size", type=int, default=3, help="gridsize value for edge_attack (default: 3)")
parser.add_argument("--iterations", type=int, default=10, help="Number of iterations for edge_attack (default: 10)")
...
def edge_attack_yolo(model_train, model_inf, device, image_path, image_id, epsilon=1, iterations=10, grid_size=3, target_confidence=0.25):
    ...
    binary_edge_mask = binary_edge_mask.to(device) 
    ...
    adv_image.save(f'edge_attack_images/{name}/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/adv_image_{image_id}.jpg')
...
os.makedirs(f'edge_attack_images/{name}/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/', exist_ok=True)
...
for sample in dataset[:100]:
    ...
    if not os.path.exists(f'edge_attack_images/{name}/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/adv_image_{image_id}.jpg'):
        
        image_path = sample.filepath
        edge_attack_yolo(model_train, model_inf, device, image_path, image_id, epsilon=epsilon, iterations=iterations, grid_size=gridsize)
        
    image = Image.open(f'edge_attack_images/{name}/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/adv_image_{image_id}.jpg')
...
os.makedirs(f'predictions/edge_attack/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/', exist_ok=True)
# Save predictions
with open(f"predictions/edge_attack/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/predictions_{name}.json", "w") as f:
...
# Load predictions
coco_dt = coco_gt.loadRes(f"predictions/edge_attack/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/predictions_{name}.json")
...
os.makedirs(f'evaluation/edge_attack/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/', exist_ok=True)
# Save the captured output to a file
with open(f"evaluation/edge_attack/epsilon_{epsilon}_iterations_{iterations}_gridsize_{gridsize}/evaluation_results_{name}.txt", "w") as f:
