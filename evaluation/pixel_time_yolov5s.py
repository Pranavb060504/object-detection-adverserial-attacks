...
from edge_attack_pixels_time import compute_pixels_time_edge_yolo
from dpattack_pixels_time import compute_pixels_time_dpattack_yolo
from tqdm import tqdm

# Parse command-line arguments
parser = argparse.ArgumentParser(description="Run faster-rcnn model inference on COCO dataset")
parser.add_argument("--epsilon", type=float, default=0.3, help="Perturbation magnitude for DPattack (default: 0.3)")
...
for sample in tqdm(dataset[:max_samples]):
    ...
    num_pixel, t = compute_pixels_time_dpattack_yolo(model_train, model_inf, device, sample.filepath, image_id, args.epsilon, args.iterations, args.grid_size) 
    ...
    num_pixel, t = compute_pixels_time_edge_yolo(model_train, model_inf, device, sample.filepath, image_id, args.epsilon, args.iterations, args.grid_size)
...
with open(f'num_pixels_time_epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_3_yolo.json', 'w') as f:
