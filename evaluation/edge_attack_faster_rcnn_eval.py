...
# Argument parser
parser = argparse.ArgumentParser(description="Run edge attack on Faster R-CNN model")
...
def edge_faster_rcnn(image_path, image_id, epsilon=1, iterations=10, grid_size=3, target_confidence=0.25, confidence_threshold=0.5, nms_threshold=0.3):
    ...
    M = torch.zeros_like(org_tensor, device=device)
    for box in boxes:
    ...
    delta = torch.zeros_like(org_tensor, requires_grad=True).to(device)
    ...
    for _ in tqdm(range(iterations)):
        ...
        for i in range(len(output[0]['scores'])):
            ...
            loss += torch.max(torch.tensor(0.0, device=device), confidence - target_confidence)
    ...
    adv_image = transforms.ToPILImage()(adv_image_tensor.squeeze(0).cpu())
    
    save_path = f'edge_attack_images/faster_rcnn/epsilon_{epsilon}_iterations_{iterations}_gridsize_{grid_size}/'
    os.makedirs(save_path, exist_ok=True)
    adv_image.save(f'{save_path}/adv_image_{image_id}.jpg')   
    
predictions = []
os.makedirs(f'edge_attack_images/faster_rcnn/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/', exist_ok=True)

for sample in dataset:
    
    image_id = int(sample.filepath.split('\\')[-1].split('.')[0])
    
    if not os.path.exists(f'edge_attack_images/faster_rcnn/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/adv_image_{image_id}.jpg'):
        
        image_path = sample.filepath
        edge_faster_rcnn(image_path, epsilon=args.epsilon, iterations=args.iterations, grid_size=args.grid_size, image_id=image_id)

    # Perform inference on the adversarial image
    image, boxes, labels, scores = detect_objects(f'edge_attack_images/faster_rcnn/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/adv_image_{image_id}.jpg', model)
...
os.makedirs(f'predictions/edge_attack/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/', exist_ok=True)

# Save predictions
with open(f"predictions/edge_attack/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/predictions_faster_rcnn.json", "w") as f:
...
# Load predictions
coco_dt = coco_gt.loadRes(f"predictions/edge_attack/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/predictions_faster_rcnn.json")
...
os.makedirs(f'evaluation/edge_attack/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/', exist_ok=True)
# Save the captured output to a file
with open(f"evaluation/edge_attack/epsilon_{args.epsilon}_iterations_{args.iterations}_gridsize_{args.grid_size}/evaluation_results_faster_rcnn.txt", "w") as f:
